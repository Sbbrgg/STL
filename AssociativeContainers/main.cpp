...
void main()
{
	...
	for (std::map<std::string, std::list<std::string>>::const_iterator it = dictionary.begin(); it != dictionary.end(); ++it)
	{
		cout << it->first << " - ";
		std::list<std::string>::const_iterator list_it = it->second.begin();
		while (list_it != it->second.end())
		{
			cout << *list_it;
			++list_it;
			if (list_it != it->second.end()) cout << ", ";
		}
		cout << endl;
	}
	cout << delimiter;
